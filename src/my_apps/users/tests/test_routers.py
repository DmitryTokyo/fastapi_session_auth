...
@pytest.mark.parametrize(
    'signin_email, verify_password_result, expected_status, expected_response_url_path',
    [
        ('exist@test.com', True, status.HTTP_302_FOUND, '{url}/app'),
        ('exist@test.com', False, status.HTTP_401_UNAUTHORIZED, None),
        ('non_exist@test.com', False, status.HTTP_401_UNAUTHORIZED, None),
    ],
)
async def test_signin_user(
    client, test_server_url, test_session, mocker, verify_password_result, expected_status, signin_email,
    expected_response_url_path,
):
    ...
    expected_url = URL(expected_response_url_path.format(url=test_server_url)) if expected_response_url_path else None
